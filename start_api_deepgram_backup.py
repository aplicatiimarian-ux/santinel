...
# --------------------------------------------------------------------------- #
#  Live transcription (FAZA 23.2) — thin, stateless proxy to Deepgram.        #
#  Audio bytes are read, forwarded, and discarded; nothing is persisted.      #
#  Switched from Groq Whisper -> Deepgram Sept 5, 2026: Groq hallucinated on  #
#  short (~4s) isolated-utterance segments; Deepgram is built for this case  #
#  and returns per-word confidence we use to filter low-confidence junk.     #
# --------------------------------------------------------------------------- #

DEEPGRAM_API_KEY = os.getenv("DEEPGRAM_API_KEY", "").strip()
DEEPGRAM_STT_MODEL = os.getenv("DEEPGRAM_STT_MODEL", "nova-2").strip()
DEEPGRAM_STT_URL = "https://api.deepgram.com/v1/listen"
DEEPGRAM_MIN_CONFIDENCE = float(os.getenv("DEEPGRAM_MIN_CONFIDENCE", "0.55"))
MAX_AUDIO_BYTES = 25 * 1024 * 1024  # generous per-segment cap
...
async def transcribe(
    ...
    """Live speech-to-text: proxy one audio segment to Deepgram.

    The web app's Live Coaching module posts ~4s webm/opus segments here while
    recording. Stateless — the bytes are forwarded to Deepgram and dropped.
    Deepgram returns per-utterance confidence; segments below
    DEEPGRAM_MIN_CONFIDENCE are treated as silence/noise (empty text) rather
    than risking a hallucinated phrase reaching the transcript.
    """
    print(f"[DEBUG] /api/transcribe called")
    print(f"[DEBUG] file={file}")
    print(f"[DEBUG] lang={lang}")
    if not DEEPGRAM_API_KEY:
        raise HTTPException(
            status_code=503,
            detail="transcription not configured (DEEPGRAM_API_KEY missing)",
        )

    audio = await file.read()
    if not audio:
        return {"text": "", "model": DEEPGRAM_STT_MODEL}
    ...
    print(f"[DEBUG] Deepgram status={resp.status_code}, text={resp.text[:300]}")
    ...
    try:
        ...
        alt = channels[0]["alternatives"][0] if channels and channels[0].get("alternatives") else {}
        text = (alt.get("transcript") or "").strip()
        confidence = alt.get("confidence")
    except (ValueError, KeyError, IndexError):
        raise HTTPException(status_code=502, detail="transcription returned unexpected shape")

    if text and confidence is not None and confidence < DEEPGRAM_MIN_CONFIDENCE:
        # Low-confidence result — most likely noise Deepgram guessed at a
        # word for. Drop the text but return 200 (not an error, just empty).
        return {"text": "", "model": DEEPGRAM_STT_MODEL, "confidence": confidence, "dropped_low_confidence": True}

    return {"text": text, "model": DEEPGRAM_STT_MODEL, "confidence": confidence}
