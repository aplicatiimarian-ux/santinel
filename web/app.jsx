...
// Debug panel removed — focus on core MediaRecorder fix for mobile
...
// Live transcription (FAZA 23.2) — POST one audio segment to the Deepgram
// proxy (switched from Groq Sept 5, 2026 — see [[santinel]] STT STRATEGY).
// Relative URL → rides the Vite `/api` proxy (same-origin, no CORS).
// Returns { text, confidence, droppedLowConfidence } — confidence lets the
// caller log/inspect Deepgram's own certainty, on top of client-side VAD.
async function postTranscribe(blob, lang) {
  ...
  return {
    ...
    droppedLowConfidence: !!(data && data.dropped_low_confidence),
  };
}
...
const t = {
  en: {
    ...
    liveSubtitle: 'Record the call live — mic audio is transcribed by Deepgram as you speak, and SANTINEL re-analyzes every 4-5 seconds.',
    ...
    latency: 'Latency',
    // live transcription (FAZA 23.2 — Deepgram)
    whisperLabel: 'Deepgram',
    whisperReady: 'ready',
    ...
    sttHybridNote: 'Instant captions from the browser; Deepgram refines each segment for the analysis.',
    webCaptionsOff: 'Instant captions unavailable here — Deepgram still transcribes every few seconds.',
    // speaker detection
  ...
  ro: {
    ...
    liveSubtitle: 'Înregistrează apelul în direct — audio-ul de la microfon este transcris de Deepgram pe măsură ce vorbești, iar SANTINEL reanalizează la fiecare 4-5 secunde.',
    ...
    sttHybridNote: 'Subtitrări instant din browser; Deepgram rafinează fiecare segment pentru analiză.',
    webCaptionsOff: 'Subtitrările instant nu sunt disponibile aici — Deepgram tot transcrie la câteva secunde.',
    // speaker detection
...
// --- Live transcription (FAZA 23.2) — Deepgram (switched from Groq Sept 5, 2026) ---
const WHISPER_SEG_MS = 4000;        // rotate the MediaRecorder every 4s -> one Deepgram call
const WHISPER_MIN_BLOB = 2500;      // ignore near-empty segments (bytes) — reduces empty/noise calls
const WHISPER_MAX_QUEUE = 4;        // cap the backlog; drop oldest if Deepgram falls behind
const WHISPER_HALLUCINATION_BLACKLIST = new Set([
  // Left in place as a second line of defense on top of Deepgram's confidence
  // filtering — these were Groq's characteristic hallucinations; Deepgram
  // shouldn't produce them, but the blacklist costs nothing to keep.
  'thank you', 'thanks', "you're welcome", "thanks for watching",
  'good bye', 'goodbye', 'bye bye',
  'hello', 'hi there',
  'well',
  ...
  'mulțumesc pentru subscribe',
]);

// --- Voice Activity Detection (VAD) — filter silence before sending to Deepgram ---
// Mobile microphones are typically quieter; detect device and adjust thresholds
...
const detectVoiceActivity = async (audioBlob) => {
  try {
    const arrayBuffer = await audioBlob.arrayBuffer();
    const audioContext = new (window.AudioContext || window.webkitAudioContext)();
    const audioBuffer = await audioContext.decodeAudioData(arrayBuffer);
    const samples = audioBuffer.getChannelData(0); // mono or first channel
    ...
    if (rms < VAD_ENERGY_THRESHOLD) {
      console.log(`[SANTINEL VAD] REJECT: too quiet (RMS=${rms.toFixed(6)} < ${VAD_ENERGY_THRESHOLD})`);
      return false;
    }
    ...
    // Voice has peaks (consonants); pure noise is flat
    const peakRatio = peak / rms;
    if (peakRatio < VAD_MIN_PEAK_RATIO) {
      console.log(`[SANTINEL VAD] REJECT: no peaks (peak/RMS=${peakRatio.toFixed(2)} < ${VAD_MIN_PEAK_RATIO})`);
      return false;
    }

    // Count silent samples (below noise floor ~0.001)
    const noiseFloor = 0.001;
    let silentCount = 0;
    for (let i = 0; i < samples.length; i++) {
      if (Math.abs(samples[i]) < noiseFloor) silentCount++;
    }
    const silentRatio = silentCount / samples.length;
    if (silentRatio > VAD_SILENCE_RATIO) {
      console.log(`[SANTINEL VAD] REJECT: mostly silence (${(silentRatio * 100).toFixed(1)}% > ${VAD_SILENCE_RATIO * 100}%)`);
      return false;
    }

    console.log(`[SANTINEL VAD] ACCEPT: RMS=${rms.toFixed(6)}, peak/RMS=${peakRatio.toFixed(2)}, silent=${(silentRatio * 100).toFixed(1)}%`);
    return true;
  } catch (err) {
    console.warn('[SANTINEL VAD] error analyzing blob:', err);
    return true; // On error, let it through (better to send than miss)
  }
};
...
const pickRecorderMime = () => {
  ...
  // Try each MIME type; if none supported, return empty (use default)
  for (const mime of WHISPER_MIME_CANDIDATES) {
    try {
      if (MediaRecorder.isTypeSupported && MediaRecorder.isTypeSupported(mime)) {
        console.log(`[SANTINEL] MediaRecorder MIME selected: ${mime}`);
        return mime;
      }
    } catch (e) {
      // Ignore errors from isTypeSupported
    }
  }
  console.log(`[SANTINEL] No MIME type supported, using default`);
  return '';
};
...
function LiveCoaching({ lang }) {
  ...
  const applyWhisperText = useCallback((rawText, speaker) => {
    ...
    console.log(`[SANTINEL live] applyWhisperText received: "${text}"`);
    if (!text) return;

    // Filter: ignore known hallucinations (blacklist approach allows legitimate short words)
    // Step 1: Normalize the text comprehensively
    const textNorm = text
      .toLowerCase()
      .replace(/[.,!?;:\s]+$/g, '')  // strip trailing punctuation & spaces
      .replace(/\s+/g, ' ')           // normalize internal spaces (multiple → single)
      .trim();

    console.log(`[SANTINEL live] normalized for blacklist: "${textNorm}"`);

    // Step 2: Check blacklist
    if (WHISPER_HALLUCINATION_BLACKLIST.has(textNorm)) {
      console.log(`[SANTINEL live] ✗ BLACKLIST MATCH: ignoring hallucination "${text}"`);
      return;
    }

    // Step 3: Check if any blacklist entry is a substring (catches variants)
    for (const entry of WHISPER_HALLUCINATION_BLACKLIST) {
      if (textNorm.includes(entry) || entry.includes(textNorm)) {
        console.log(`[SANTINEL live] ✗ SUBSTRING MATCH: "${entry}" in "${textNorm}" — ignoring`);
        return;
      }
    }

    console.log(`[SANTINEL live] ✓ ACCEPT: "${text}"`);
  ...
  const drainWhisperQueue = useCallback(async () => {
    ...
    try {
      while (whisperQueueRef.current.length) {
        ...
        try {
          console.log(`[SANTINEL live] Sending blob to Deepgram: ${(blob.size / 1024).toFixed(1)}KB from ${speaker}`);
          const { text, confidence, droppedLowConfidence } = await postTranscribe(blob, langRef.current);
          if (droppedLowConfidence) {
            console.log(`[SANTINEL live] Deepgram low-confidence (${confidence?.toFixed(2)}) — dropped`);
          } else {
            console.log(`[SANTINEL live] Deepgram returned: "${text}" (confidence=${confidence?.toFixed(2) ?? 'n/a'})`);
          }
  ...
  const enqueueWhisperBlob = useCallback(async (blob, speaker) => {
    if (!blob || blob.size < WHISPER_MIN_BLOB) {
      console.log(`[SANTINEL live] ✗ REJECT blob: size=${blob?.size || 0}B < ${WHISPER_MIN_BLOB}B (min=${WHISPER_MIN_BLOB}B)`);
      return;
    }
    console.log(`[SANTINEL live] blob received: ${(blob.size / 1024).toFixed(1)}KB from ${speaker}, running VAD...`);
    // Check voice activity before enqueueing
    const hasVoice = await detectVoiceActivity(blob);
    if (!hasVoice) {
      console.log(`[SANTINEL live] ✗ REJECT blob: VAD failed`);
      return;
    }
    const q = whisperQueueRef.current;
    q.push({ blob, speaker });
    console.log(`[SANTINEL live] ✓ blob PASS VAD, enqueued (queue size: ${q.length}/${WHISPER_MAX_QUEUE})`);
    while (q.length > WHISPER_MAX_QUEUE) {
      const dropped = q.shift();
      console.log(`[SANTINEL live] ⚠ queue overflow, dropped oldest segment`);
    }
    drainWhisperQueue();
  }, [drainWhisperQueue]);

  function rotateWhisperSegment() {
    console.log('[SANTINEL whisper] rotateWhisperSegment called');
    const mr = mediaRecorderRef.current;
    mediaRecorderRef.current = null;
    if (mr && mr.state !== 'inactive') {
      console.log('[SANTINEL whisper] Stopping MediaRecorder, state:', mr.state);
      try { mr.stop(); } catch (e) { /* ignore */ }   // -> ondataavailable enqueues the segment
    } else {
      console.log('[SANTINEL whisper] MediaRecorder inactive or null');
    }
    if (whisperOnRef.current) {
      console.log('[SANTINEL whisper] Starting new segment...');
      startWhisperSegment();
    }
    if (whisperOnRef.current) segTimerRef.current = setTimeout(rotateWhisperSegment, WHISPER_SEG_MS);
  }
  ...
  const startRecording = async () => {
    ...
    try {
      console.log('[SANTINEL mic] Requesting getUserMedia...');
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      console.log('[SANTINEL mic] ✓ getUserMedia success, stream tracks:', stream.getAudioTracks().length);
      ...
      console.warn('[SANTINEL mic] ✗ Meter microphone unavailable:', err.name, err.message);
      setMicState('denied');
    }
    ...
    if (streamRef.current && typeof MediaRecorder !== 'undefined') {
      ...
      console.log('[SANTINEL whisper] Selected MIME type:', mimeType || '(default)');
      recorderMimeRef.current = mimeType;
      whisperOnRef.current = true;
      setWhisperState('idle');
      startWhisperSegment();
    } else {
      console.warn('[SANTINEL whisper] ✗ Stream unavailable or MediaRecorder not supported');
      whisperOnRef.current = false;
      setWhisperState('off');
    }
